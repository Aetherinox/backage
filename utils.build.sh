...
# #
#   @project              Docker Image › Backage › utils.build.sh
#   @repo                 https://github.com/aetherinox/backage
#   @file                 utils.build.sh
...
#
#   @image:github         ghcr.io/aetherinox/backage:latest
#                         ghcr.io/aetherinox/backage:2025
#                         ghcr.io/aetherinox/backage:2025.6
#                         ghcr.io/aetherinox/backage:2025.6.20
#
#   @image:dockerhub      aetherinox/backage:latest
#                         aetherinox/backage:2025
#                         aetherinox/backage:2025.6
#                         aetherinox/backage:2025.6.20
#
...
#                                 If you attempt to build your Backage docker image on Linux, and have windows CRLF
...
#                               --build-arg IMAGE_BUILDDATE=20250620 \
#                               --build-arg IMAGE_VERSION=2025.6.20 \
...
#                               --tag aetherinox/backage:2025.6.20 \
...
image_version_date_1digit=$(date -u +%Y.%-m.%-d)                                    #  2025.6.20
image_version_date_2digit=$(date -u +%Y.%m.%d)                                      #  2025.06.19
...
while [ $# -gt 0 ]; do
    case "$1" in
        -v|--ver|--version)
            if [[ "$1" != *=* ]]; then shift; fi
            image_version="${1#*=}"                                                 #  2025.06.19
            IFS="." read major minor patch <<< "${image_version}"
            image_version_1digit=${major}                                           #  2025
            image_version_2digit=${major}.${minor}                                  #  2025.06
            ;;
        -n|--name)
            if [[ "$1" != *=* ]]; then shift; fi
            image_name="${1#*=}"
